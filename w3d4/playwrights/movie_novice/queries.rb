...
def find_angelina
  ...
  answer = Actor.find_by(name:'Angelina Jolie')
  answer
end
...
def top_titles
  ...
  Movie.select(:id, :title).where('score >= ?',9.0)
end

def star_wars
  #display the id, title and year of each Star Wars movie in movies.
  # hint: use 'select' and 'where'
  Movie.select(:id, :title, :yr).where('title LIKE ?','%tar Wars%')
end
